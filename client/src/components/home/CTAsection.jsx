...
const CTASection = ()  => {
    ...
    return (
        ...
        >
            ...
            <motion.button
            onClick={() => navigate(!user ? (setShowLogin(true), setState("register")) : "/features")}
            className="px-12 py-3 border border-purple-400/60 text-white hover:bg-purple-500/20  rounded-full text-sm mt-4"
                initial={{ y: 80, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                viewport={{ once: true }}
                transition={{ type: "spring", stiffness: 280, damping: 70, mass: 1 }}
