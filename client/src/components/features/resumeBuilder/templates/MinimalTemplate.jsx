  ...
  return (
    <div className="mx-auto h-full w-full max-w-205 rounded-lg border border-slate-200 bg-white p-6 text-slate-800">
      <div className="mb-6 flex items-start justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h1 className="wrap-break-word text-xl font-semibold tracking-tight sm:text-2xl">{personal.full_name || 'Your Name'}</h1>
          <p className="mt-1 wrap-break-word text-[11px] font-medium uppercase tracking-[0.18em]" style={{ color: accentColor }}>
            {personal.profession || 'Professional Title'}
          </p>
        </div>
        <div className="max-w-38 text-right text-[10px] leading-5 text-slate-600 sm:text-[11px]">
          ...
          <div className="mt-2 space-y-2 text-[11px] text-slate-700">
            {(education.length ? education : [{ degree: '', institution: '', location: '', start_date: '', end_date: '', score: '' }]).map((item, index) => (
              <div key={index} className="rounded-md border border-slate-100 bg-slate-50/60 p-2">
                ...
                <p className="text-[10px] text-slate-500 mt-1">{(item.start_date || item.end_date) ? `${item.start_date || ''} - ${item.end_date || ''}` : 'Year'}</p>
              </div>
            ))}
          </div>
        </aside>
        ...
        <main className="md:col-span-8">
          ...
          <div className="mt-2 space-y-3">
            {(experience.length ? experience : [{ job_title: '', company_name: '', location: '', start_date: '', end_date: '', description: '' }]).map((item, index) => (
              <div key={index} className="relative rounded-md border border-slate-100 bg-slate-50/50 p-3 pl-4">
                ...
                <p className="mt-1.5 wrap-break-word text-[11px] text-slate-700 whitespace-pre-wrap">{item.description || 'Describe your key work impact and achievements.'}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-2 space-y-2">
            {(projects.length ? projects : [{ title: '', link: '', tech_stack: [], description: '' }]).map((item, index) => (
              <div key={index} className="rounded-md border border-slate-100 bg-white p-3 shadow-sm">
